...
fn render_jj(data: &Option<JJData>) -> Vec<Segment> {
    let c_blue = Color::Ansi256(69);
    let c_purple = Color::Ansi256(140);
    let c_magenta = Color::Ansi256(163);
    let c_gray = Color::Ansi256(250);
    let c_dark = Color::Ansi256(238);
    let c_green = Color::Ansi256(71);

    let Some(data) = data else {
        return Vec::new(); // not in a jj repo: contribute nothing
    };
    ...
    segs.push(Segment::new(prefix, c_magenta));
...
pub fn component() -> Component<Option<JJData>> {
    Component {
        enabled: Dynamic::dynamic(|_ctx| jj_root_exists()),
        get_data: Box::new(get_jj_data),
        render: Box::new(render_jj),
